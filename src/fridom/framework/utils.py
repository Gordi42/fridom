...
def sync_with_gdrive(func: callable,
                     filenames: list[str],
                     ) -> callable:
    """
    A decorator for downloading / uploading files to Google Drive.

    If the returned function is called in the doc build process, the function 
    will not call the decorated function, but instead download the file from
    Google Drive. If the returned function is called in the normal execution
    process, the function will call the decorated function and upload the file
    to Google Drive.

    Parameters
    ----------
    `func` : `callable() -> None`
        The function to decorate (should create the files specified in `filenames`).
    """
    # check if we are in the doc build process
    is_build = False # TODO: implement this
    if is_build:
        return download_from_gdrive(filenames)
    else:
        func()
        return upload_to_gdrive(filenames)

def download_from_gdrive(filenames: list[str]):
    return

def upload_to_gdrive(filenames: list[str]):
    # Get the settings from the environment variables
    import os
    creds_path = os.getenv('FRIDOM_GDRIVE_CREDENTIALS')
    directory_id = os.getenv('FRIDOM_GDRIVE_DIRECTORY_ID')
    file_id_table = os.getenv('FRIDOM_GDRIVE_FILE_ID_TABLE')

    if creds_path is None or directory_id is None or file_id_table is None:
        logger.warning("Not all necessary gdrive environment variables are set.")
        logger.warning("Continue without uploading to Google Drive.")
        return
    ...
    try:
        ...
        logger.warning("pydrive2 is not installed.")
        logger.warning("Continue without uploading to Google Drive.")
        return

    # import the file id table
    try:
        import json
        with open(file_id_table, 'r') as f:
            file_ids = json.load(f)
    except:
        logger.warning("Could not load the file id table.")
        file_id_table = {}
        return

    gauth = GoogleAuth()
    gauth.credentials = ServiceAccountCredentials.from_json_keyfile_name(
        pkg_resources.resource_filename(__name__, creds_path),
        scopes=['https://www.googleapis.com/auth/drive'])
    drive = GoogleDrive(gauth)

    for f in filenames:
        file = drive.CreateFile({'parents': [{'id': directory_id}], 
                                'title': f})
    ...
    # save the file id table
    with open(file_id_table, 'w') as f:
