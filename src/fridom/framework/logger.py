...
# here, which is not recommended. Nevertheless, we disable the pylint
# warning here.
def verbose(message: str, *args, **kwargs) -> None:
    """Log a message with level VERBOSE."""
    if log.isEnabledFor(15):
        log._log(15, message, args, **kwargs)  # pylint: disable=protected-access

def notice(message: str, *args, **kwargs) -> None:
    """Log a message with level NOTICE."""
    if log.isEnabledFor(25):
        log._log(25, message, args, **kwargs)  # pylint: disable=protected-access
