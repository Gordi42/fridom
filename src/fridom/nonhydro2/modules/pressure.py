...
is multiplied directly by the 0-d ``1/dsqr`` leaf (``Symbol * scalar``:
a ``Symbol`` accepts a Python number or a 0-d array), never via
    ...
    def _laplacian_symbol(self, dsqr: jax.Array) -> Symbol:
        ...
        for term in self._terms:
            ...
            if term.bound_axis == self._vertical:
                # the traced 1/dsqr scaled in at the symbol level; a 0-d
                # jax leaf multiplies a Symbol directly (Symbol * scalar)
                sym = sym * (1.0 / dsqr)
