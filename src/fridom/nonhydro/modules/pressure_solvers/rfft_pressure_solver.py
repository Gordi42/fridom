...
class RFFTPressureSolver(fr.modules.Module):
    ...
    def _setup_k_squared_inv(self) -> None:
        ...
        @fr.utils.jaxjit
        def _setup_k_squared_inv() -> ncp.ndarray:
            grid = self.mset.grid
            ...
            return - fr.config.ncp.where(k_squared == 0, 0, k_squared_inv)
    ...
    def _setup_default_transform_functions(self) -> None:
        def forward_transform(
                x: ncp.ndarray, axes: set[int] | None = None) -> ncp.ndarray:
            axes = axes or set(range(self.mset.grid.n_dims))
            axes = set(axes)
            dct_axes = axes & self.dct_axes
            fft_axes = axes & self.fft_axes
            if len(dct_axes) > 0:
                x = scp.fft.dctn(x, type=2, axes=list(dct_axes))
            if len(fft_axes) > 0:
                x = ncp.fft.fftn(x, axes=list(fft_axes))
            return x
        ...
        def backward_transform(
            ...
            fft_axes = axes & self.fft_axes
            if len(fft_axes) > 0:
                x = ncp.fft.ifftn(x, axes=list(fft_axes))
            if len(dct_axes) > 0:
                x = scp.fft.idctn(x, type=2, axes=list(dct_axes))
            return x
    ...
    def _setup_transform_functions_single_gpu(self) -> None:
        dd = self.mset.grid.domain_decomp
        def forward_transform(x: ncp.ndarray) -> ncp.ndarray:
            x = dd.unpad(x)
            if len(self.dct_axes) > 0:
                x = scp.fft.dctn(x, type=2, axes=list(self.dct_axes))
            if len(self.fft_axes) > 0:
                x = ncp.fft.rfftn(x, axes=list(self.fft_axes))
            return x

        def backward_transform(x: ncp.ndarray) -> ncp.ndarray:
            if len(self.fft_axes) > 0:
                x = ncp.fft.irfftn(x, axes=list(self.fft_axes))
            if len(self.dct_axes) > 0:
                x = scp.fft.idctn(x, type=2, axes=list(self.dct_axes))
    ...
    def _setup_transform_functions_multiple_gpus(self) -> None:
        def forward_transform(
            ...
            if rfft_axis in axes:
                x = ncp.fft.rfft(x, axis=rfft_axis)
            if len(fft_axes) > 0:
                x = ncp.fft.fftn(x, axes=list(fft_axes))
            return x
        ...
        def backward_transform(
            ...
            if len(fft_axes) > 0:
                x = ncp.fft.ifftn(x, axes=list(fft_axes))
            if rfft_axis in axes:
                x = ncp.fft.irfft(x, axis=rfft_axis)
            if len(dct_axes) > 0:
                x = scp.fft.idctn(x, type=2, axes=list(dct_axes))
            return x
