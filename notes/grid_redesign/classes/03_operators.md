...
`grid.wavenumbers` / metric fields
...
src/fridom/framework/grid2/operators/
    ...
    flux_diff.py          # FluxDifference, FVDerivative
    spectral.py           # SpectralDerivative, PhaseShift
    transform.py          # Transform (ABC)
    fourier.py            # Fourier
    trig.py               # Sine, Cosine            (designed-for)
    chebyshev.py          # Chebyshev               (designed-for)
...
The only dynamic-leaf carrier in this cluster is `Symbol`
(`_data`) and, transitively, the grid reference held by transforms.
...
kernels are unary), and a linear hierarchy keeps `isinstance` checks
in the halo-accounting trace trivial.
...
class UnaryOperator(Operator, ABC):
    ...
    @abstractmethod
    def __call__(
        self, f: ScalarField, axis: str | None = None,
    ) -> ScalarField:
        """Apply the operator to ``f``; ``axis`` names the factor."""
        ...
```
...
class BinaryOperator(Operator, ABC):
    """Operator applied to two fields: ``op(f, g)``."""
    ...
    @abstractmethod
    def __call__(self, f: ScalarField, g: ScalarField) -> ScalarField:
        """Apply the binary operator to ``(f, g)``."""
        ...
```

`codomain(a, b)` applies the two sanctioned implicit exceptions of the
strict algebra before demanding equality: `ConstantSpace` broadcast
...
class SeparableOperator(UnaryOperator, ABC):
    ...
    def __call__(
        ...
        """Apply the kernel along ``axis`` (required if ambiguous)."""
        ...
...
- `__call__` resolves the factor: if the field has exactly one
...
class Symbol:
    ...
    def __init__(
        ...
        data: jax.Array,
        codomain: FunctionSpace | TensorProductSpace | None = None,
    ) -> None:
    ...
    # ------------------------------------------------------------
    @property
    def space(self) -> TensorProductSpace:
        """Domain coefficient space of the diagonal."""
        ...

    @property
    def codomain(self) -> TensorProductSpace:
    ...
    def __pow__(self, p: int | float) -> Symbol: ...
...
- **A `Symbol` is not a `ScalarField`** and not a `ScalarField`
  ...
  a diagonal map.
- **Broadcast across product factors via `ConstantSpace`** (§3.3):
  a per-factor symbol returned by
  `fd.eigenvalues(grid, fourier_x)` lives on
  `Fourier(x) ⊗ Constant(y) ⊗ Constant(z)`; `+` and `*` of symbols on
  broadcast-compatible spaces produce full-product symbols. The
  discrete Laplacian is the broadcast sum over factors (sketch 4.6).
...
class LinearInterp(SeparableOperator):
    ...
    def __init__(self) -> None:
        """Create the two-point interpolation kernel."""
        ...

    def codomain(self, domain: FunctionSpace) -> FunctionSpace:
        """interp: Center <-> Right (periodic); Outer/Inner -> Center."""
        ...
...
- Fixed codomain per §3.4: the registered operator fixes its own
  ...
  A `target=` constructor knob was rejected — alternative targets are
  different operator instances a module registers explicitly.
...
class LinearReconstruction(SeparableOperator):
    ...
    def __init__(self) -> None:
        """Create the two-point reconstruction kernel."""
        ...

    def codomain(self, domain: FunctionSpace) -> FunctionSpace:
        """CellAvg -> Right | Outer; Right/Outer -> CellAvg; FaceAvg dual."""
        ...
...
- Covers both directions of sketch 4.7's table at second order:
  average-to-point (`CellAvg -> Outer`, Shu two-point mean) and
...
| Kind | concrete (final) |
| Pytree | static |
| Iteration | 1 (parity with today's `weno_interpolation.py`) |
...
- Upwinding is a *pair* of biased instances; flux-splitting advection
  modules hold both and select by velocity sign. The alternative — a
  velocity-consuming ternary operator — was rejected as a
  module-level (physics) concern, consistent with §3.9
...
class FluxDifference(SeparableOperator):
    ...
    def codomain(self, domain: FunctionSpace) -> FunctionSpace:
        """flux_diff: Outer(n+1) | Right(n) | Inner(n-1) -> CellAvg(n)."""
        ...
...
- On Fourier factors it is the diagonal `i k` multiply — origin
  preserved (spectral differentiation does not stagger); this is the
  iteration-1 surface. On sine/cosine coefficient factors it is the
  honest bc-flipping signature `d/dx : SineCoeff -> CosineCoeff`
  (§3.2), replacing today's `SpectralDiff` metadata mutation —
  designed-for, with the `Sine`/`Cosine` transforms. On Chebyshev
  factors it is the coefficient recurrence (designed-for, with the
  Chebyshev spaces). `layout "local"`: the recurrence couples all
  modes of the
...
class PhaseShift(SeparableOperator):
    """Exact e^{i k s dx} shift between per-origin Fourier spaces."""
...
- Exact, unitary, and diagonal: applying it *is* applying its own
  symbol. The default registry entry
  `("interp", Fourier(origin=Right)) ->
...
|---|---|
| Kind | ABC |
| Pytree | static structure; holds the grid reference (whose dynamic leaves flow through jaxify) |
| Iteration | 1 |
...
class Transform(UnaryOperator, ABC):
    ...
    def __call__(
        ...
        """Alias of ``forward`` (registry-uniform application)."""
        ...
...
- **Dealiasing is a property of the transform** (§3.12): with
  ...
  binds the grid.
- `__call__` aliasing `forward` keeps `("transform", space)` registry
  entries applicable through the uniform operator calling convention;
  user code is expected to write `forward`/`backward`.
...
| Pytree | static structure (bound grid) |
| Iteration | designed-for (with doc 01's `SineSpace`/`CosineSpace`; only `mx.fourier` is day one, §10.2/§10.6) |
...
- **Caveat:** the old cartesian grid runs DST/DCT paths on bounded
  axes today (`cartesian/fft.py`), so demoting these transforms means
  iteration 1 does not fully cover bounded-axis spectral parity;
  restoring that parity (`Sine`/`Cosine` plus their registry rows) is
  the **first post-iteration-1 item**.
...
| Kind | concrete (final) |
| Pytree | static structure (bound grid) |
| Iteration | designed-for (with `ChebyshevMesh` / Galerkin spaces, §6.2) |
...
space's BC structure (§3.6); `requirements` declares `layout "local"`
along its axes (no distributed Chebyshev transform planned — the
§6.2 decomposition ships x/y sharded, z local).
...
Also the iteration-1 `("multiply", CellAvg)` default: the pointwise
product of cell averages is the standard **second-order shortcut**
(identifying averages with midpoint values, O(dx^2) — the §3.9
identification made explicit). The true quadrature product
(reconstruct, multiply, re-average) remains designed-for and replaces
this entry when higher-order FV lands.
...
call site (binary ops pass `to=` when their `codomain` unites a
constant factor with a full one) — a recorded widening of the
per-factor signature, analogous to the composed-operator widening.
On nodal/average targets the embedding is exact replication (halo 0,
layout "any", iteration 1). On coefficient targets it is the exact
delta embedding (the constant lands in the k = 0 / mean mode, scaled
by the basis normalization) — designed-for.
...
shortcut on average spaces, no coefficient-space entries (a quotient
or power of spectra has no representation-independent meaning; go
through a transform) — so they live alongside it in `products.py`.

| | |
|---|---|
| Kind | concrete (final), three classes |
| Pytree | static |
| Iteration | 1 (`divide`/`power` on nodal + average; `abs` on nodal) |
| Concept refs | §3.11 |
| Module | `grid2.operators.products` |
...
class Hadamard(BinaryOperator):
    ...
    def __call__(
        self, f: ScalarField, *others: ScalarField,
    ) -> ScalarField:
...
- Elementwise in coefficients = convolution in physical space; it has
  ...
  `"multiply"` by design.
- Variadic call: sketch 4.9's amplitude-and-phase scaling is
  `Hadamard()(f, a, r)` — associative elementwise products fuse into
  one call. The instance-call spelling is the **accepted decision**
...
class Integral(SeparableOperator):
    ...
    def requirements(
        ...
        """halo = 0; needs a collective sum across shards."""
        ...
```
...
Exact on average
spaces. The result broadcasts back via `ConstantSpace` (§3.3), so
`f - f.integrate("x")` stays in the strict algebra. There is no
unweighted `sum` operator (`f.data.sum()` is the escape hatch).
...
class CumulativeIntegral(SeparableOperator):
    ...
    def codomain(self, domain: FunctionSpace) -> FunctionSpace:
        """cumint: CellAvg(n) -> Outer(n+1); Center -> Right/Inner."""
        ...
...
composes per-factor symbols internally (`bwd @ fwd` per factor,
summed via `ConstantSpace` broadcast — sketch 4.6); it raises
...
class Divergence(UnaryOperator):
    ...
    def __call__(
        self, f: VectorField, axis: str | None = None,
    ) -> ScalarField:
...
|---|---|
| Kind | final concrete |
| Pytree | static container (held operators may carry leaves) |
| Iteration | 1 |
...
- **Product-key resolution** (`space` a `TensorProductSpace`; this is
  ...
  The iteration-1 tables only ever need form 2 (the grid seeds
  per-factor `("multiply", Center(m))` entries all holding the one
  `CollocationProduct` instance); form 1 exists so a spectral grid
  can pin `Convolution` to one specific product space.
...
| `("diff", Sine/Cosine coeff)` | `SpectralDerivative()` *(designed-for)* | `Sine <-> Cosine` |
| `("interp", nodal)` | `LinearInterp()` | staggering partner |
| `("interp", Fourier(o != center))` | `PhaseShift(to=NodeSet.CENTER)` | `-> Fourier(origin=Center)` |
| `("reconstruct", CellAvg)` | `LinearReconstruction()` | `CellAvg -> Right/Outer` |
| `("reconstruct", Right/Outer)` | `LinearReconstruction()` | `-> CellAvg` |
| `("flux_diff", Right/Outer/Inner)` | `FluxDifference()` | `-> CellAvg` |
| `("transform", Real nodal)` | `Fourier(grid, ...)` (rfft path) | `-> Fourier(o)` half-spectrum |
| `("transform", Complex nodal)` | same instance (fft path) | `-> Fourier(o)` full |
| `("transform", bounded Dirichlet)` | `Sine(grid, ...)` *(designed-for)* | `-> DST-I/II coeff` |
| `("transform", bounded Neumann)` | `Cosine(grid, ...)` *(designed-for)* | `-> DCT coeff` |
| `("multiply", nodal)` | `CollocationProduct()` | same space |
| `("multiply", Fourier(o))` | `Convolution(padded Fourier)` *(designed-for)* | same space |
| `("multiply", CellAvg)` | `CollocationProduct()` (2nd-order shortcut; true quadrature product *designed-for*) | same space |
| `("broadcast", ConstantSpace)` | `ConstantBroadcast()` (nodal/average targets; coefficient delta embedding *designed-for*) | `ConstantSpace -> target factor` |
| `("divide", nodal/CellAvg)` | `Divide()` | same space |
| `("power", nodal/CellAvg)` | `Power()` | same space |
| `("abs", nodal)` | `Abs()` | `-> fr.Real variant` |
| `("integrate", any factor)` | `Integral()` | `-> ConstantSpace` |
| `("cumint", Center/CellAvg)` | `CumulativeIntegral()` | `-> face space` |
...
space instance of its meshes (spaces are interned, so this is a small
finite table).
...
`("power", ...)`, `("abs", ...)` the same way; binary `codomain`
...
4. **Frozen registry view.** Resolved in substance: doc 04 specifies
   the registry is treated as frozen after decomposition negotiation
   and is part of the grid's static dispatch identity. The remaining
   detail is only whether the grid hands out a distinct frozen *view
   object* or relies on the convention (`__setitem__` pre-negotiation
   only).

Resolved since the first draft: the refined-mesh handle for padded
transforms is no longer open — doc 01 now defines
`StructuredMesh1D.refined(factor)` (iteration 1 on `IntervalMesh`,
`Fraction` factor), cited in the `Transform` notes above.
