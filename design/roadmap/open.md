...
| 3.9 | **Multi-device compile and execution cost** — *not scheduled; the numbers are unacceptable and known* | The mapped pressure solve is correct under decomposition but disproportionately expensive there. Measured 2026-07-13, forced-4 host devices on cpu, 16x16 mapped solve at 12 iterations: **compile 9.1 s** (single device: 0.7 s) and **194 ms per jitted call** — **694x** the single-device 0.28 ms — against a problem only 4-way sharded. HLO is 52 200 lines vs 4 750. The `lax.scan` conversion (3.6) removed the O(iterations) blowup, so this is *not* the unrolled-loop cost: it is what SPMD partitioning of the sharded metric-scaled stencil chains and the CG's cross-shard dot-product reductions cost per iteration. Forced host devices exaggerate it (four "devices" contending for one cpu, collectives with no interconnect), so **the first task is to measure on real devices** — a gpu/multi-host run — before concluding how much is an artifact of the test harness and how much is real. Then: count the collectives per CG iteration (the halo exchange plus two all-reduces) and see whether the reductions can be batched or the preconditioner made shard-local. Gates the credibility of 3.3. |
