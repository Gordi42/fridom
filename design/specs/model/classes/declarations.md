...
5. **`TIME_STEP` canonical string**: `"stepper.dt"` is proposed here — confirm at 2.2/2.4 when the stepper joins the binding table.
6. **`USE_PROVIDED` corner**: `update_parameters` targeting a name whose reference was suppressed by an explicit constructor value — error or no-op? Settle with the D4 lifecycle implementation (2.3/2.4).
7. **`Ramp.reversed()` construction**: the mirrored-window translation given here implements V-S2 and the retrace law for all curves — confirm against the ported OB legs in the 2.7 tolerance-based cutover tests.
