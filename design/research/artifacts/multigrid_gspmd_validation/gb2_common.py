"""Shared builders for the multigrid GSPMD / immersed measurement legs.

Mirrors the GB-2 mapped case (design/research/multigrid_gb2_wallclock.md)
and the kernel-study immersed case (multigrid_kernel_study.md Phase A).
Import AFTER setting JAX_PLATFORMS / XLA_FLAGS / CUDA_VISIBLE_DEVICES in
the environment (fridom touches the backend at import).
"""
...
def build_mapped_solver(n, *, preconditioner="multigrid", method="auto",
    ...
    # obtain the exact grid + pressure space from a built model so the
    # dispatch registry and decomposition match the production solve
...
# ================================================================
#  Immersed case (kernel study Phase A: slope geometry, n=128)
# ================================================================
def slope_fraction(x, y, z):
    """Tilted-slope wet fraction (canonical immersed slope, genuine partials).

    Mirrors tests/nonhydro2/test_immersed_pressure.py's ``slope`` geometry
    (a tilted cut surface varying in x, y and z), on a walled z in (0,1).
    Returns the WET (open) fraction; genuine partial cells via order-4
    quadrature in ImmersedDomain.
    """
    return jnp.clip(
...
def immersed_mean_free_rhs(solver, grid, space):
    """A smooth wet-supported mean-free rhs (projected onto wet-mean)."""
    rhs = grid.create_field(
        space,
        init=lambda x, y, z: jnp.exp(
            -((x - 3.0) ** 2 + (y - 3.0) ** 2 + (z - 0.5) ** 2) * 3.0))
    return solver.projection(rhs)
