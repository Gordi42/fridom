...
from FIDOM.NonHydrostatic.BoundaryConditions import UBoundary, VBoundary, WBoundary, BBoundary, TriplePeriodic
...
class State(StateBase):
    ...
    def ver_vort_x(self) -> FieldVariable:
        ...
        vort = ((w.diff_forward(1)*dsqr/dy -  v.diff_forward(2)/dz)
                ).ave(-1, 1).ave(-1, 2)
    ...
    def ver_vort_y(self) -> FieldVariable:
        ...
        # calculate the vertical vorticity
        vort = ((u.diff_forward(2)/dz - dsqr * w.diff_forward(0)/dx)
                ).ave(-1, 0).ave(-1, 2)
    ...
    def pot_vort(self) -> FieldVariable:
        ...
        buo_grad_x = b.diff_forward(0).ave(-1,0) * Ro / (N0**2 *dx)
        buo_grad_y = b.diff_forward(1).ave(-1,1) * Ro / (N0**2 *dy)
        buo_grad_z = b.diff_forward(2).ave(-1,2) * Ro / (N0**2 *dz)
    ...
    def pecl_h(self) -> float:
        ...
        """
        umax = self.cp.max(self.u**2 + self.v**2 + self.v**2)
