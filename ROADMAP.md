...
| 3.5 | **Finite-volume nonhydro** | Move the nonhydro model to the average family (`CellAvg` scalars, face-normal velocities — decision FV-D2 **option A**, owner 2026-07-12). Staged: the four FV symbol rows (the long pole — they block `SpectralSolve` and hence the pressure solve), the missing conversion rows, an **FV tracer slice** (which is where the payoff lands: high-order WENO on walled domains), then the C-grid profile with **bitwise parity** as the gate. Do **not** flip the default wholesale first — walls and mapped grids work today and would regress. Scoping record, with the nine ranked operator gaps and the four decisions: [`design/plans/active/fv_nonhydro_scoping.md`](design/plans/active/fv_nonhydro_scoping.md). |
| 3.6 | **CG compile cost: `lax.scan` the Krylov loop** | The mapped pressure CG is an **unrolled** fixed-iteration loop, so tracing and XLA compilation are O(iterations): ~0.24 s/iteration, 25k HLO lines at the default 30 iterations, and a jitted forced-4 solve does not finish compiling. (The *metric* re-derivation per iteration — the other half of the cost — is fixed as of `479e2965`.) Convert to `lax.scan` (not `fori_loop`: `jax.grad` must keep working) by carrying raw arrays plus a static space and rebuilding fields inside the body, which sidesteps the `ScalarField.halo_valid` treedef-stability obstacle. **Needs a plan before implementation** (owner, 2026-07-12). |
| 3.8 | **Generalized adiabatic ramping** — *idea, unscheduled* | An `AdiabaticRamping` base transform that ramps declared parameters from a start to an end value over a ramp period (continuous stage-time `Ramp` evaluation, shape `exp`/`pow`/`cos`/`lin`/callable), with **`OptimalBalance` as a subclass** contributing only the balancing policy (Rossby ramp, base point, the `(I - P)` leg, the `FixedPoint`). Also buys adiabatic spin-up / parameter continuation. Depends on 2.8. Record: [`design/plans/active/adiabatic_ramping.md`](design/plans/active/adiabatic_ramping.md). |
| 3.7 | **Spherical nonhydro** — *long-term* | The 3D spherical chart (`X(lon, lat, h)` with `h` a height/depth coordinate, so the metric comes out diagonal and w = dh/dt is already physical) needs the C2 chart metrics and the C3 elliptic machinery to meet: the pressure operator becomes the Laplace–Beltrami on the chart — still SPD under the sqrt(g)-weighted product, so the PCG structure carries over, but the operator assembly must be written. **Not the first 3D-spherical consumer**: a hydrostatic model needs no pressure solve and is the likelier first use (ROADMAP 3.1, greenfield). |
