...
def test_lint_warns_when_the_filter_keeps_linear_terms(sw_setup):
    ...
    with pytest.warns(UserWarning, match="not quadratic"):
        BalanceExpansion(
            model, order=0, nonlinear=fr.model.term_predicates.explicit)
...
def test_lint_is_quiet_on_the_default_filter_and_optout(sw_setup):
    ...
    with warnings.catch_warnings():
        ...
        BalanceExpansion(
            model, order=0,
            nonlinear=fr.model.term_predicates.explicit, lint=False)
...
def _nh_walled_model_at(device_ids, n=8):
    """Return a walled-vertical (rigid-lid z) nonhydro model."""
    meshes = tuple(
        fr.spatial.meshes.IntervalMesh(
            n, (0.0, 1.0 if name == "z" else 2 * np.pi),
...
@pytest.mark.multi_device
def test_nh_walled_balance_runs_on_a_sharded_axis(forced_devices):
    # Wave B: on a walled-vertical grid the analytic V / W / L_w^{-1}
    # operators become fused walled per-mode matrix applies (Fourier
    # transpose + local trig + union-lattice matrix) when the default
    # layout shards a periodic transform axis; the balanced state matches
    # the replicated one-device reference to floating point across orders.
    if forced_devices is not None:
        assert jax.device_count() == forced_devices
    n = 8
    rng = np.random.default_rng(4)
    fields = {c: 0.15 * rng.standard_normal(
        (n, n, n - 1 if c == "w" else n)) for c in NH_COMPONENTS}
    many = _nh_walled_model_at(None, n)
    many.set_fields(**fields)
    z_many = nh.State({c: many.state[c] for c in NH_COMPONENTS})
    assert z_many["u"]._data.sharding.spec[0] == "devices"
    one = _nh_walled_model_at((0,), n)
    one.set_fields(**fields)
    z_one = nh.State({c: one.state[c] for c in NH_COMPONENTS})
    for order in (0, 1, 2):
        bal_many = BalanceExpansion(many, order=order, lint=False)(z_many)
