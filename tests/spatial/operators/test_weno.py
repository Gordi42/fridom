...
def invariant(a, b):
    ...
    reductions relative to the single-device program, so the
    reconstruction matches only to a tight absolute tolerance, not
    bit-for-bit. A real device-count bug is O(1) or NaN, far above the
    tolerance.
    """
