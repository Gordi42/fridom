...
def test_the_closure_is_localized_to_the_surface_cell():
    # constancy diagnostic: A(b = const) probes the advection's
    # effective discrete divergence. It is machine-zero in every
    # interior cell (the 3D flow is discretely divergence-free there),
    # and non-zero only in the top (surface) cell, where the restriction
    # has dropped the surface velocity w(0) -> the zero-boundary-flux
    # closure. The bottom cell stays exact (w = 0 is seeded there).
    grid = make_grid()
...
def test_advection_preserves_the_semidiscrete_energy_skew():
    ...
    # Adding CenteredAdvection must not degrade it: the flux-form
    # advection is energy-orthogonal in the M metric (the interior flow
    # is divergence-free and the surface closure integrates to zero by
    # the horizontal-mean symmetry of a smooth field), so the full-RHS
    # skew stays at the machine-precision level with the vertical leg
    # active. This is the sense in which the H2 quadratic energy is
    # conserved to time-discretization accuracy.
...
def test_advection_tendency_is_energy_orthogonal():
    ...
    # property, the nonhydro relative_energy_rates pattern).
    metric = EnergyMetric(hy.energy.hydrostatic_energy_weights(1.0, 1.0))
    grid = make_grid()
    model = make_model(grid)
    _set(model, grid,
