...
def test_multigrid_on_a_stretched_column_still_raises():
    # kept as-is (a later branch teaches the V-cycle the measure
    # widths): construction succeeds, the V-cycle assembly raises when
    # it reaches the stretched factor's absent uniform spacing
    grid, mx, ms = build_grid()
    solver = MappedPressureSolver(
        grid, cell_space(mx, ms), iterations=5,
        weights={"sigma": 1.0 / DSQR}, preconditioner="multigrid")
    with pytest.raises(NotImplementedError,
                       match="no uniform cell width"):
        solver.krylov()
